...
function cleanIfNewVersion(cache, version) {
  if (!version) return;
  return cache.get('appVersion').then(function (oldVersion) {
    if (oldVersion !== version) {
      return cache.reset(); // unfortunately multi cache doesn't return a promise
...
function tryStoreVersion(cache, version) {
  ...
  return cache.set('appVersion', version, {
    ttl: null
  }).then(function () {
...
module.exports = function cacheRenderer(nuxt, config) {
  ...
  renderer.renderRoute = function (route, context) {
    ...
    return cache.get(cacheKey).then(function (cachedResult) {
      if (cachedResult) {
